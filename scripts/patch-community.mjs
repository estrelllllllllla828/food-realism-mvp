...
const mockApiPatch = `export const MOCK_COMMUNITY_POSTS: CommunityPost[] = [
  {
    id: 'p1',
    food_id: '10026',
    food_name: '\u8c6a\u534e\u5bff\u53f8\u62fc\u76d8',
...
const mockApi = fs.readFileSync(path.join(root, 'src/api/mockApi.ts'), 'utf8')
const importLine = "import type {\n  CommunityComment,\n  CommunityPost,\n  CommunityPostDetail,"
const updatedImport = mockApi.includes('CommunityComment')
  ? mockApi
  : mockApi.replace(
      'import type {\n  CommunityPost,',
      importLine,
...
const postsStart = updatedImport.indexOf('export const MOCK_COMMUNITY_POSTS')
const postsEnd = updatedImport.indexOf('export const MOCK_USER')
const fetchStart = updatedImport.indexOf('export async function fetchCommunityPosts')
const fetchEnd = updatedImport.indexOf('export async function fetchUserProfile')

const beforePosts = updatedImport.slice(0, postsStart)
const afterPosts = updatedImport.slice(postsEnd, fetchStart)
const afterFetch = updatedImport.slice(fetchEnd)

const newFetch = `export async function fetchCommunityPosts(): Promise<CommunityPost[]> {
  await delay(350)
...
fs.writeFileSync(
  path.join(root, 'src/api/mockApi.ts'),
  beforePosts + mockApiPatch + '\n' + afterPosts + newFetch + afterFetch,
  'utf8',
)

console.log('mockApi updated')
